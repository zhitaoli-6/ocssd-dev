...
struct pblk_sec_meta {
	//u64 reserved;
	__le64 lba[4]; //add by kan
    u64 d_idx[4]; //add by kan for debug
    u64 d_mod[4]; //add by kan for debug
    u64 d_ppa[4]; //add by kan for debug
    u64 d_sec_stripe; //add by kan for debug
    u64 d_nr_secs; //add by kan for debug
    u64 resv[46]; //add by kan
}; //512B
...
// fast lookup for lba lookup of a line group
struct group_l2p_node {
	struct rb_node node;
	// key
	u64 lba;
	// value
...
struct pblk_md_line_group_set {
	...
	struct group_l2p_node **nodes_buffer;
